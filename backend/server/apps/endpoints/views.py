...
class PredictView(views.APIView):
    def post(self, request, endpoint_name, format = None):
        ...
        alg_ver = self.request.query_params.get("version")
        algs = MLAlgorithm.objects.filter(#parent_endpoint__name = endpoint_name,  ???????????????????? bez tego dziala
                                          status__status = alg_status, 
                                          status__active = True)
        if alg_ver != None:
            algs = algs.filter(version = alg_ver)
        if len(algs) == 0:
            return Response({"status": "Error", "message": "ML algorithm is not available"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if len(algs) != 1 and alg_status != "ab_testing":
            return Response(
        ...
        alg_index = 0
        if alg_status == "ab_testing" and len(algs) > 1:
            if rand() < 0.5 :
                alg_index = 0
            else :
                alg_index = 1
